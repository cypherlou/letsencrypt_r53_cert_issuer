...
class IssueCert(object):
    ...
    def issue_cert(self, domain: str) -> None:
        ...
        self.certificate_key_file = "{}_certificate{}".format(
            filename, extension)

        if os.path.exists(self.key_file):
            self.log.info("loading Let's Encrypt account keys")
            self.account = sewer.crypto.AcemAccount.read_pem(
                self.account_key_file)
        ...
        self.log.info(
            "requesting certificate for {}".format(
                self.naked_domain))
        self.pem = "{}\n{}".format(
            client.get_certificate(),
            self.certificate.to_pem().decode())

        self.log.info(
            "writing account credentials to {}".format(
                self.account_key_file))
        self.account.write_pem(self.account_key_file)
        self.log.info(
            "writing certificate key to {}".format(
